	...
	#region Methods
		...
		override public void Draw( Graphics g, GraphPane pane, int pos, float scaleFactor  )
		{
			if ( _isVisible )
			{
                ...
                if (symbolsDrawn == null)
                {
                    symbolsDrawn = new Dictionary<Symbol, int>();
                    for (int i = 0; i < Points.Count; i++)
                    {
                        if (Points[i].Symbol == null && !symbolsDrawn.ContainsKey(Symbol))
                        {
                            Symbol.Draw(g, pane, this, scaleFactor, IsSelected);
                            symbolsDrawn[Symbol] = 0;
                        }
                        else if (Points[i].Symbol != null && !symbolsDrawn.ContainsKey(Points[i].Symbol))
                        {
                            Points[i].Symbol.Draw(g, pane, this, scaleFactor, IsSelected);
                            symbolsDrawn[Points[i].Symbol] = 0;
                        }
                    }
                }
                else
                {
                    foreach (Symbol symbol in symbolsDrawn.Keys)
                    {
                        symbol.Draw(g, pane, this, scaleFactor, IsSelected);
                    }
                }
	    ...
	    private Dictionary<Symbol, int> symbolsDrawn;
